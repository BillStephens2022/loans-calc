...
import {
  getLoanAccountingExampleById,
  getJournalEntriesByExampleId,
} from "../../lib/api";
...
const AccountingExampleDetail: React.FC = () => {
    ...
    if (accountingExampleId) {
      const fetchLoanAccountingExampleDetails = async () => {
        try {
          ...
          const exampleData = await getLoanAccountingExampleById(
            accountingExampleId
          );
          ...
          const journalEntriesData = await getJournalEntriesByExampleId(
            accountingExampleId
          );
  ...
  if (loading || example === null) {
    // Loading state while fetching accounting example details or if example is null: SHow Loader component
    return <Loader loadingText="Fetching Selected Example..."/>; 
  }
