...
const Fronting = () => {
  ...
  useEffect(() => {
    if (formData) {
      ...
      const globalAvailability = globalCommitment - globalFundedLoans - globalLettersOfCredit;
      ...
      const swinglineSublimitAvailability = swinglineSublimit - swinglinesFundedByYourBank;
      const lcSublimitAvailability = lcSublimit - lcsIssuedByYourBank;
      const totalSublimitAvailability = swinglineSublimitAvailability + lcSublimitAvailability;
      ...
      if (hasAvailability && totalSublimitAvailability > 0) {
        if (swinglineSublimitAvailability > 0) {
          unfundedSwinglineFrontingExposure = otherLendersShare * swinglineSublimitAvailability;
          fundedSwinglineFrontingExposure = otherLendersShare * swinglinesFundedByYourBank;
        }
        if (lcSublimitAvailability > 0) {
          unissuedLCFrontingExposure = otherLendersShare * lcSublimitAvailability;
      ...
      {showFrontingExposure && (
        <div>
          ...
          <ul className={classes.frontingExposureList}>
            <li className={classes.frontingExposureListItem}>Unfunded Swingline Fronting: {formatAmount(frontingExposure.unfundedSwinglineFrontingExposure)}</li>
            <li className={classes.frontingExposureListItem}>Funded Swingline Fronting: {formatAmount(frontingExposure.fundedSwinglineFrontingExposure)}</li>
            <li className={classes.frontingExposureListItem}>Unissued/Unutilized LC Fronting: {formatAmount(frontingExposure.unissuedLCFrontingExposure)}</li>
            <li className={classes.frontingExposureListItem}>Issued LC Fronting: {formatAmount(frontingExposure.issuedLCFrontingExposure)}</li>
          </ul>
        </div>
      )}
    </main>
